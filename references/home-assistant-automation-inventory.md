...
- [x] Dashboard buttons created in HA dashboard `Neima Home` at `/neima-home/home`; Family Room Modes buttons call `script.turn_on` for `script.family_room_evening` and `script.family_room_movie`. Previous `/ned-home/home` dashboard path is hidden from the sidebar as an old-path rollback copy.
