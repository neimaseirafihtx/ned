...
- [x] Dashboard buttons created in HA dashboard `Neima Home` at `/neima-home/home`; `Second Floor Scenes` buttons call `script.turn_on` for `script.family_room_evening`, `script.family_room_movie`, and `script.all_house_lights_off`. Dashboard expanded with room/status cards for Family Room, Kitchen/Dining, Entryway, Neima’s Room, Listening Room, Guestroom, Office/Garage, and Exterior. Previous `/ned-home/home` dashboard path is hidden from the sidebar as an old-path rollback copy. Mobile/dashboard cleanup now omits display-specific entities like `media_player.75_the_frame` and `media_player.bedroom_display` so the phone view stays cleaner.
